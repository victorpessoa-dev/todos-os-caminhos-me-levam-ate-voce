...
export default function AdminGallery() {
    ...
    const [form, setForm] = useState({ title: "", image_url: "", image_alt: "" });
    ...
    async function handleAdd() {
        if (!form.image_url.trim()) return setMessage("URL da imagem e obrigatoria.");
        setSaving(true);
        setMessage("");
        try {
    ...
    async function handleDelete(img) {
        ...
        if (!ok1) return;
        const ok2 = window.confirm("Confirme novamente: deseja realmente excluir esta imagem?");
        if (!ok2) return;
        try {
    ...
    return (
        <div className="min-h-screen bg-gray-50 px-4 py-6 sm:px-6 sm:py-8">
            <div className="mx-auto max-w-4xl">
                <h1 className="mb-6 text-2xl font-semibold text-gray-900 sm:text-3xl">Galeria</h1>

                <div className="admin-card mb-8 space-y-4 p-4 sm:p-6">
                    <h2 className="text-base font-semibold text-gray-800">Adicionar imagem</h2>
                    <div>
                ...
                {loading && (
                    <div className="py-10 text-center">
                        <div className="inline-block h-9 w-9 animate-spin rounded-full border-2 border-marsala border-t-transparent" aria-hidden />
                    </div>
                )}

                {!loading && images.length === 0 && (
                    <p className="py-12 text-center text-sm text-gray-600">Nenhuma imagem na galeria.</p>
                )}

                {!loading && images.length > 0 && (
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {images.map((img) => (
                            <div key={img.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition-colors hover:border-marsala/20">
                                <img
                                    src={sanitizeImageUrl(img.image_url) || "/placeholder.jpg"}
                                ...
                                <div className="flex flex-col items-start gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
                                    <p className="break-words text-sm text-gray-700">{img.title || "Sem titulo"}</p>
                                    <button
                                        type="button"
